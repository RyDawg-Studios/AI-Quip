from data.aigame.objects.ui.rolelist import RoleList
...
class RevealRolesLevel(Level):
    def __init__(self, man, pde):
        ...
        for inx, player in enumerate(self.pde.game.playerinfo.keys()):
            ...
            text = f"{name} - {role}"
            self.objectManager.add_object(obj=RoleList(man=man,pde=pde, position=[320, 300 + inx*32], scale=[128,32],useCenterForPosition=True, text=text))
